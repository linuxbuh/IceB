...
/*15.07.2015	30.08.2005	Белых А.И.	rasrnn_p.c
...
int rasrnn_p7(class xrnn_poi *data,
int kol_strok,
int period_type,
...
if(ff_dot != NULL)
   ibros=iceb_t_insfil(imafalx.ravno(),ff_dot);
...
//  fprintf(ff_lst_rozkor,"%3d %-*.*s %-*s %02d.%02d.%04d %-*s %-6s %02d.%02d.%04d %-*.*s %10.2f %10.2f\n",
    ...
    if(mz != 0)
     {
      ...
      rpvnn_pi(data->dn,data->mn,data->gn,gettext("Всего за месяц"),itogo_mes1,itogo_ou,&kol_strok,ff);
      if(ff_dot != NULL)
        rpvnn_pi(data->dn,data->mn,data->gn,gettext("Всего за месяц"),itogo_mes2,itogo_ou,&ibros,ff_dot);
...
//    OSTANOV();
  ...
  if(korrekt == 1)/*Если в виде документа присутствует буква У - то это коректирующий документ*/
   {
    ...
    iceb_tu_kolbait(20,naim_kontr.ravno()),
    naim_kontr.ravno(),
    iceb_tu_kolbait(10,row[10]),row[10],
    dv,mv,gv,
  ...
  rasrnn_p_svod(data->dn,data->mn,data->gn,nomdok.ravno(),naim_kontr.ravno(),stroka_xml,&kol_strok,ff);
  if(ff_dot != NULL)
    rasrnn_p_svod(data->dn,data->mn,data->gn,nomdok.ravno(),naim_kontr.ravno(),stroka_xml_dot,&ibros,ff_dot);
      ...
      if(dod5->innn1d1.find(row[10]) < 0)
       {
        ...
        if(SRAV(row[10],"400000000000",0) == 0)
...
fprintf(ff_prot,"%*s:%10.2f %8.2f\n",iceb_tu_kolbait(27,gettext("Итого")),gettext("Итого"),sumd_pr[0],sumnds_pr[0]);
...
if(data->mn != data->mk)
 {
  rpvnn_pi(data->dn,data->mn,data->gn,gettext("Общий итог"),itogo_ob1,itogo_ou,&kol_strok,ff);
  if(ff_dot != NULL)
    rpvnn_pi(data->dn,data->mn,data->gn,gettext("Общий итог"),itogo_ob2,itogo_ou,&ibros,ff_dot);
 }
...
if(sravmydat(dn,mn,gn,1,1,2010) >= 0)
 {
  if(iceb_tu_strlen(naim_kontr) > 20 || iceb_tu_strlen(nomdok) > 9)
   {
