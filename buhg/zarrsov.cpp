  ...
  for(int nom=0; nom < svod_otvr->kod.kolih(); nom++)
   {
    ...
    fprintf(ff,"%-*.*s|",
    iceb_tu_kolbait(20,iceb_tu_adrsimv((nom_str+1)*20,strsql)),
    iceb_tu_kolbait(20,iceb_tu_adrsimv((nom_str+1)*20,strsql)),
    iceb_tu_adrsimv((nom_str+1)*20,strsql));
   }  
  fprintf(ff,"%20s|\n","");
 }
//fprintf(ff,"%14s|\n","");
...
for(int nom=0; nom <= svod_otvr->kod.kolih(); nom++)
 {
  fprintf(ff,"%-*.*s|%-*.*s|%-*.*s|",
  iceb_tu_kolbait(6,gettext("Рабочие")),
  iceb_tu_kolbait(6,gettext("Рабочие")),
  gettext("Рабочие"),
  iceb_tu_kolbait(6,gettext("Календарные")),
  iceb_tu_kolbait(6,gettext("Календарные")),
  gettext("Календарные"),
  iceb_tu_kolbait(6,gettext("Часы")),
  iceb_tu_kolbait(6,gettext("Часы")),
  gettext("Часы"));
 }
fprintf(ff,"\n");
