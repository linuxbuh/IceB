/*$Id:$*/
/*16.05.2017	16.05.2017	Белых А.И.	uosik818.cpp
...
poltekdat(&dt,&mt,&gt);
poiin(inv_nom,dt,mt,gt,&kod_podr,&kod_mo);
...
bsiz(inv_nom,kod_podr,dt,mt,gt,&bal_st,NULL);
...
iceb_t_vrvr("uosik_begin.alx",&rv,fil.ff); /*вставка реквизитов в шапку документа*/

int tipz=0;
double balst=0.;
double iznos=0.;
class iceb_tu_str naim_oper("");
int nomer_str=0;
...
//  datdok.new_plus(iceb_tu_datzap(row[0]));
  ...
  iceb_tu_kolbait(26,naim_oper.ravno()),  
  iceb_tu_kolbait(26,naim_oper.ravno()),  
  naim_oper.ravno(),
  balst,
  "",
...
if(kolstr == 0)
 {
  iceb_t_insfil("uosik_end.alx",fil.ff);
 }
else
 {
  while(cur.read_cursor(&row) != 0)
