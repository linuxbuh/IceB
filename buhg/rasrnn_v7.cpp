...
/*19.07.2014	30.08.2005	Белых А.И.	rasrnn_v7.c
...
int rasrnn_v7(class xrnn_poi *data,
int kol_strok,
int period_type,
...
class iceb_tu_str imafalx("");


if(sravmydat(data->dn,data->mn,data->gn,1,1,2010) < 0)
  imafalx.new_plus("rpvnn2.alx");


if(sravmydat(data->dn,data->mn,data->gn,1,1,2010) >= 0 && sravmydat(data->dn,data->mn,data->gn,1,1,2011) < 0)
  imafalx.new_plus("rpvnn2_1.alx");

if(sravmydat(data->dn,data->mn,data->gn,1,1,2011) >= 0 && sravmydat(data->dn,data->mn,data->gn,1,3,2013) < 0)
  imafalx.new_plus("rpvnn2_2.alx");

if(sravmydat(data->dn,data->mn,data->gn,1,3,2013) >= 0)
  imafalx.new_plus("rpvnn2_3.alx");

//Вставляем шапку 
kol_strok+=iceb_t_insfil(imafalx.ravno(),ff);
if(ff_dot != NULL)
...
short dv,mv,gv;
class iceb_tu_str metka_7("");
#define RAMER_MAS_KOR 7
while(cur.read_cursor(&row) != 0)
 {
    ...
    if(mz != 0)
     {
      ...
      rpvnn_vi(data->dn,data->mn,data->gn,gettext("Всего за месяц"),itogo_o1,itogo_ou,&kol_strok,ff);
      if(ff_dot != NULL)
        rpvnn_vi(data->dn,data->mn,data->gn,gettext("Всего за месяц"),itogo_o2,itogo_ou,&ibros,ff_dot);
     }
  ...
  rasrnn_v_str(data->dn,data->mn,data->gn,nomdok.ravno(),naim_kontr.ravno(),metka_dok,sumd1,snds1,&kol_strok,itogo_mes,stroka_xml,ff);
  if(ff_dot != NULL)
    rasrnn_v_str(data->dn,data->mn,data->gn,nomdok.ravno(),naim_kontr.ravno(),metka_dok,sumd-sumd1,snds-snds1,&kol_strok,itogo_mes,stroka_xml_dot,ff_dot);
...
if(data->mn != data->mk)
 {
  rpvnn_vi(data->dn,data->mn,data->gn,gettext("Общий итог"),itogo_o1,itogo_ou,&kol_strok,ff);
  if(ff_dot != NULL)
    rpvnn_vi(data->dn,data->mn,data->gn,gettext("Общий итог"),itogo_o2,itogo_ou,&ibros,ff_dot);
...
if(nomer_str_rozkor > 0)
 {
  ...
  iceb_tu_kolbait(96,gettext("Итого")),gettext("Итого"),
...
if(proc_dot != 0.)
 {
  ...
  itogo_o1[0]=okrug(itogo_o1[0],0.01);
  itogo_o1[1]=okrug(itogo_o1[1],0.01);
  itogo_o1[2]=okrug(itogo_o1[2],0.01);
  itogo_o1[3]=okrug(itogo_o1[3],0.01);
  itogo_o1[4]=okrug(itogo_o1[4],0.01);
  itogo_o1[5]=okrug(itogo_o1[5],0.01);
...
if(sravmydat(dn,mn,gn,1,3,2013) >= 0) /*13 колонка*/
  fprintf(ff,"%10s|","");

fprintf(ff,"\n");
if(sravmydat(dn,mn,gn,1,1,2010) >= 0)
 {
  if(iceb_tu_strlen(naim_kontr) > 20 || iceb_tu_strlen(nomdok) > 8)
   {
