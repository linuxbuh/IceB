/*$Id: buh_rh.c,v 5.7 2013/05/17 14:55:56 sasa Exp $*/
/*02.11.2009	07.11.2006	Белых А.И.	buh_rh.c
Распечатка шахматки
Возвращаем итог по шахматке
*/
#include "buhg.h"

int buh_rh_sumk(int sk,int kolih_d,int kolih_k,class masiv_din_double *sum_pr,int metka);
void buh_rh_ps(int kolih_d,int kolih_k,class masiv_din_double *sum_pr,int metka_dk,FILE *ff_dv);


double buh_rh(int metka_dkv, //0-дебет по вертикали 1-кредит по вертикали
class SPISOK *sheta_deb,
class SPISOK *sheta_kre,
class masiv_din_double *sum_pr,
FILE *ff)
{
...
if(metka_dkv == 0)
 {
  ...
  fprintf(ff,"%-*.*s|",iceb_tu_kolbait(10,gettext("Счёт")),iceb_tu_kolbait(10,gettext("Счёт")),gettext("Счёт"));

  for(int ii=0; ii < kolih_k; ii++)
   {
    if(buh_rh_sumk(ii,kolih_d,kolih_k,sum_pr,0) == 0)
       continue;
    fprintf(ff,"%-*.*s|",iceb_tu_kolbait(10,sheta_kre->ravno(ii)),iceb_tu_kolbait(10,sheta_kre->ravno(ii)),sheta_kre->ravno(ii));
   }
  ...
  for(int sd=0; sd < kolih_d; sd++)
   {
    ...
    fprintf(ff,"%-*.*s|",iceb_tu_kolbait(10,sheta_deb->ravno(sd)),iceb_tu_kolbait(10,sheta_deb->ravno(sd)),sheta_deb->ravno(sd));
  ...
  fprintf(ff,"%*.*s|",iceb_tu_kolbait(10,gettext("Итого")),iceb_tu_kolbait(10,gettext("Итого")),gettext("Итого"));
  itogo_gor=0.;
  for(int sk=0; sk < kolih_k; sk++)
   {
    if(buh_rh_sumk(sk, kolih_d,kolih_k,sum_pr,0) == 0)
  ...
  for(int ii=0; ii < kolih_d; ii++)
   {
    if(buh_rh_sumk(ii,kolih_d,kolih_k,sum_pr,1) == 0)
       continue;
    fprintf(ff,"%-*.*s|",iceb_tu_kolbait(10,sheta_deb->ravno(ii)),iceb_tu_kolbait(10,sheta_deb->ravno(ii)),sheta_deb->ravno(ii));
   }
  ...
  for(int sk=0; sk < kolih_k; sk++)
   {
    ...
    itogo_gor=0.;
    fprintf(ff,"%-*.*s|",iceb_tu_kolbait(10,sheta_kre->ravno(sk)),iceb_tu_kolbait(10,sheta_kre->ravno(sk)),sheta_kre->ravno(sk));
    for(int sd=0; sd < kolih_d; sd++)
     {
...
/******************************/
void buh_rh_ps(int kolih_d,int kolih_k,class masiv_din_double *sum_pr,int metka_dk,FILE *ff)
{
if(metka_dk == 0)
