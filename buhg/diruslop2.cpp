/*$Id: diruslop2.c,v 5.5 2013/09/26 09:43:31 sasa Exp $*/
/*01.02.2017	30.12.2011	Белых А.И.	diruslop2.c
Просмотр списка операций для подсистемы "Учёт услуг"
*/
#include <errno.h>
#include "buhg.h"

void diruslop2_sap(int metka_rr,const char *poisk);
int diruslop2_vz(const char *kod);
void diruslop2_kk(const char *skod,const char *nkod);
int diruslop2_pvu(const char *kod);
void diruslop2_ras(class SQLCURSOR *cur,const char *poisk);


int diruslop2(int metka_rr, /*0-ввод и корректировка 1-выбор 2-читый выбор*/
...
lll:;
  ...
  diruslop2_sap(metka_rr,poisk.ravno());
...
switch (K)
 {
  ...
  case ENTER: 
    if(metka_rr != 0)
     {
      sprintf(strsql,"select naik from Usloper2 where kod='%s'",kod_opv.ravno());
  ...
  case SFK2: 
    ...
    diruslop2_vz(kod_opv.ravno());
    goto naz1;

  case FK3:       /*Удалить запись*/
    if(diruslop2_pvu(kod_opv.ravno()) != 0)
     goto naz;
    ...
    sprintf(strsql,"delete from Usloper2 where kod='%s'",kod_opv.ravno());
...
int diruslop2_vz(const char *kod_opv)
{
char strsql[512];
SQL_str row;
class SQLCURSOR cur;
...
if(kod_opv[0] != '\0') /*Корректировка записи*/
 {
  sprintf(strsql,"select * from Usloper2 where kod='%s'",kod_opv);
  if(readkey(strsql,&row,&cur) != 1)
   {
...
VV.VVOD_SPISOK_add_data(kod_kontr.ravno(),16);//5
...
if(SRAV(kod_opv,VV.data_ravno(0),0) != 0)
 {
  sprintf(strsql,"select kod from Usloper2 where kod='%s'",VV.data_ravno(0));
  if(readkey(strsql) == 1)
   {
...
if(kod_opv[0] == '\0') /*Ввод новой записи*/
 {
  sprintf(strsql,"insert into Usloper2 values ('%s','%s',%d,%ld,%d,%d,'%s','%s')",
  VV.data_ravno(0),
...
else /*Корректировка существующей*/
 {
  ...
  sprintf(strsql,"update Usloper2 set \
kod='%s',\
naik='%s',\
ktoz=%d, \
vrem=%ld, \
...
if(metka_kor_kod == 1)
  diruslop2_kk(kod_opv,VV.data_ravno(0));
...
{
char strsql[512];
sprintf(strsql,"select kodop from Usldokum where tp=2 and kodop='%s' limit 1",kod);
...
sprintf(strsql,"select oper from Prov where oper='%s' and tz=2 and kto='%s' limit 1",kod,ICEB_MP_USLUGI);
...
---------------------------------------------------------------------------\n");
fprintf(ff,gettext("\
 Код |Наименование операции         |Вид операц| Проводки |Конт|Счёт\n"));
fprintf(ff,"\
---------------------------------------------------------------------------\n");
