/*$Id: dirzarlgot.c,v 5.5 2013/05/17 14:56:00 sasa Exp $*/
/*14.07.2015	10.11.2011	Белых А.И.	dirzarlgot.c
...
void dirzarlgot_sap();
void dirzarlgot_rs(class SQLCURSOR *cur);
int dirzarlgot_vvod(int metka_r,int tabnom,const char *dndl);
...
sprintf(strsql,"select * from Zarlgot order by tn asc");
...
for(i=0;i<=kls-1;)
 {
  ...
  sprintf(strsql,"%6s|%*.*s|%7s|%6s|%1s|%2s|%.*s",
  row[0],
...
//    prosf(strsql);
  ...
  case ENTER:
    refresh();
    dirzarlgot_vvod(1,tabnom,dndl.ravno());
    goto naz1;
  ...
  case FK3:  /*удалить*/
    ...
    sprintf(strsql,"delete from Zarlgot where tn=%d and dndl='%s'",tabnom,dndl.ravno_sqldata());
...
clear();
printw("%s",gettext("Справочник льгот"));
...
mvprintw(y++,x,"3-%s",gettext("дата нач.дейст."));
mvprintw(y++,x,"4-%s",gettext("процент"));
mvprintw(y++,x,"5-%s",gettext("количество льгот"));
mvprintw(y++,x,"6-%s",gettext("код льготы"));
mvprintw(y++,x,"7-%s",gettext("коментарий"));
...
/*запись*/
/*****************************************/
int dirzarlgot_zap(int metka_r,class VVOD *VV,int tabnomz,const char *dndl)
{
char strsql[512];
short d=0,m=0,g=0;
...
if(VV->data_ravno_atoi(0) != tabnomz || (mz != m || gz != g))
 {
  sprintf(strsql,"select tn from Zarlgot where tn=%d and dndl='%04d-%02d-%02d'",VV->data_ravno_atoi(0),g,m,d);
  if(readkey(strsql) > 0)
   {
...
int dirzarlgot_vvod(int metka_r, /*0-новая запись 1-корректировка записис*/
int tabnomz,const char *dndl)
{
int N=0,K=0;
char strsql[512];
class VVOD VV(0);
...
if(metka_r == 1)
 {
  ...
  SQL_str row;
  class SQLCURSOR cur;
  sprintf(strsql,"select * from Zarlgot where tn=%d and dndl='%04d-%02d-%02d'",tabnomz,g,m,d);
  ...
  VV.data_plus(1,iceb_tu_datzap_mg(row[1]));
...
VV.VVOD_SPISOK_add_MD(gettext("Дата начала действия.(м.г).."));//1
VV.VVOD_SPISOK_add_MD(gettext("Процент....................."));//2
VV.VVOD_SPISOK_add_MD(gettext("Количество льгот............"));//3
VV.VVOD_SPISOK_add_MD(gettext("Код льготы.................."));//4
  ...
  if(K == FK2 || K == PLU) /*Записываем*/
   {
    attroff(VV.VVOD_return_cs(iceb_CFS));
    attron(VV.VVOD_return_cs(iceb_CFM));
    if(dirzarlgot_zap(metka_r,&VV,tabnomz,dndl) != 0)
      goto naz;
    break;
   }
  if(K == FK3)
...
zagolov(gettext("Справочник льгот"),0,0,0,0,0,0,iceb_t_get_pnk("00",0),ff);
...
oth.spis_naim.plus(gettext("Справочник льгот"));
