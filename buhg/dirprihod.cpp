/*$Id:$*/
/*01.12.2016	01.12.2016	Белых А.И.	dirprihod.c
Работа со справочкиком приходов в подсистеме "Материальный учёт"
*/
#include <errno.h>
#include "buhg.h"

void dirprihod_sap(int metka_rr,const char *poisk);
int dirprihod_vz(const char *kod);
void dirprihod_kk(const char *skod,const char *nkod);
int dirprihod_pvu(const char *kod);
void dirprihod_ras(class SQLCURSOR *cur,const char *poisk);


int dirprihod(int metka_rr, /*0-ввод и корректировка 1-выбор*/
...
lll:;
  ...
  dirprihod_sap(metka_rr,poisk.ravno());
...
switch (K)
 {
  ...
  case ENTER: 
    if(metka_rr != 0)
     {
      sprintf(strsql,"select naik from Prihod where kod='%s'",kod_opv.ravno());
  ...
  case SFK2: 
    ...
    dirprihod_vz(kod_opv.ravno());
    goto naz1;

  case FK3:       /*Удалить запись*/
    if(dirprihod_pvu(kod_opv.ravno()) != 0)
     goto naz;
    ...
    sprintf(strsql,"delete from Prihod where kod='%s'",kod_opv.ravno());
...
int dirprihod_vz(const char *kod_opv)
{
char strsql[512];
SQL_str row;
class SQLCURSOR cur;
...
if(kod_opv[0] != '\0') /*Корректировка записи*/
 {
  sprintf(strsql,"select * from Prihod where kod='%s'",kod_opv);
  if(readkey(strsql,&row,&cur) != 1)
   {
  ...
  if(metka_vido == 2)
   vido.new_plus(gettext("Изменение учётных данных"));
    ...
    if(N == 2) /*Вид операции*/
     {
      ...
      MENUW.VVOD_SPISOK_add_MD(gettext("Изменение учётных данных"));
      ...
      switch (metka_vido)
       {
        ...
        case 2 :
         VV.VVOD_SPISOK_zapis_data(2,gettext("Изменение учётных данных"));
         break;
       }
...
if(SRAV(kod_opv,VV.data_ravno(0),0) != 0)
 {
  sprintf(strsql,"select kod from Prihod where kod='%s'",VV.data_ravno(0));
  if(readkey(strsql) == 1)
   {
...
if(kod_opv[0] == '\0') /*Ввод новой записи*/
...
else /*Корректировка существующей*/
 {
  ...
  sprintf(strsql,"update Prihod set \
kod='%s',\
naik='%s',\
ktoz=%d, \
vrem=%ld, \
...
if(metka_kor_kod == 1)
  dirprihod_kk(kod_opv,VV.data_ravno(0));

if(kod_opv[0] == '\0')
 if(metka_vido > 0) /*Пишем парную запись в приходные операции*/
  {
   sprintf(strsql,"insert into Rashod (kod,naik,ktoz,vrem,vido,prov,kk) values ('%s','%s',%d,%ld,%d,%d,'00')",
   VV.data_ravno(0),
   VV.data_ravno_filtr(1),
   iceb_t_getuid(),time(NULL),
   metka_vido,
...
{
char strsql[512];
sprintf(strsql,"select kodop from Dokummat where tip=1 and kodop='%s' limit 1",kod);
...
if(readkey(strsql) > 0)
 {
  sprintf(strsql,"%s Reenn1 !",gettext("Удалить нельзя, потому, что есть записи в таблице"));
  iceb_t_soob(strsql);
  return(1);
 }

sprintf(strsql,"select oper from Prov where oper='%s' and tz=1 and kto='%s' limit 1",kod,ICEB_MP_MATU);
...
class iceb_tu_str vido("");
sprintf(imaf,"kodopp%d.lst",getpid());
...
----------------------------------------------------------------------------------------------\n");
fprintf(ff,gettext("\
 Код |Наименование операции         |Вид операц|Проводки|Дата и время записи| Кто записал\n"));
fprintf(ff,"\
----------------------------------------------------------------------------------------------\n");
...
while(cur->read_cursor(&row) != 0)
 {
  ...
  if(atoi(row[4]) == 1)
    vido.new_plus(gettext("Внутренняя"));
  if(atoi(row[4]) == 2)
    vido.new_plus(gettext("Изм.данных"));
