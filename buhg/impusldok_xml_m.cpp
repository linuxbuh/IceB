/*$Id:$*/
/*04.02.2015	04.02.2015	imusldok_xml_m.c
Меню шапки документа
*/
#include "buhg.h"
      ...
      if(N == 1 && VV.VVOD_SPISOK_return_data(N)[0] != '\0') /*склад*/
       {
        ...
        if(VV.data_ravno(0)[0] != '\0')  
         {
          iceb_t_nomnak(VV.data_ravno(0),VV.data_ravno(N),&rk->nomdok,1,1,0);
...
//      iceb_t_pdoc(strsql);
    ...
    case FK5:
     if(VV.data_ravno(0)[0] != '\0' || VV.data_ravno(1)[0] != '\0')  
      {
       iceb_t_nomnak(VV.data_ravno(0),VV.data_ravno(1),&rk->nomdok,1,1,0);
       VV.data_plus(2,rk->nomdok.ravno());
      }        
...
sprintf(strsql,"select kod from Uslpodr where kod=%d",rk->sklad.ravno_atoi());
if(readkey(strsql) != 1)
 {
  iceb_t_soob(gettext("Не найден код подразделения в справочнике!"));
  goto naz;
 }    
...
sprintf(strsql,"select vido from Usloper1 where kod='%s'",rk->kod_op.ravno_filtr());
...
if(rk->shetu.getdlinna() <= 1)
 {
  iceb_t_soob(gettext("Не введён счёт учёта!"));
  goto naz;
 }
...
class lock_tables lokt("LOCK TABLES Usldokum WRITE,icebuser READ");
