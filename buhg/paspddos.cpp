/* $Id: paspddos.c,v 5.22 2013/09/26 09:43:36 sasa Exp $ */
/*17.03.2012	22.06.1998	Белых А.И.	paspddos.c
...
валютных расчётов будет изменена структура текстового файла для приема
...
fprintf(ff,"%-*.*s",iceb_tu_kolbait(149,rec.naiban1.ravno()),iceb_tu_kolbait(149,rec.naiban1.ravno()),rec.naiban1.ravno()); /*Наименование банка корреспондента(если есть МФО можно пробелы)*/
...
fprintf(ff,"%-*.*s",iceb_tu_kolbait(149,rec.naior1.ravno()),iceb_tu_kolbait(149,rec.naior1.ravno()),rec.naior1.ravno()); /*Наименование корреспондента (например ДЕМО КЛИЕНТ)*/
...
/*Вид обработки (1-ПЛАТЕЖНОЕ ПОРУЧЕНИЕ, 2-ТРЕБОВАНИЕ ПОРУЧЕНИЕ)*/                                             
/************
if(SRAV(tabl,"Pltp",0) == 0)
  fprintf(ff,"%-3.3s","1");                  
if(SRAV(tabl,"Pltt",0) == 0) 
...
fprintf(ff,"%-*.*s",iceb_tu_kolbait(159,koment.ravno()),iceb_tu_kolbait(159,koment.ravno()),koment.ravno());  /*Назначение платежа (длина больше 3 символов)*/
...
perecod(1,imaf); /*Перекодируем в дос кодировку*/
