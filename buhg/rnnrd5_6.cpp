...
/*15.07.2015	19.01.2015	Белых А.И.	rnnrd5_6.c
...
void rnnrd5_6_endras(class rnn_d5 *dod5,double,int,FILE *ff_svod,FILE *ff_xml);

extern class iceb_rnfxml_data rek_zag_nn;

int rnnrd5_6(short dn,short mn,short gn,short dk,short mk,short gk,
  ...
  for(int ii=0; ii < dod5->innn_per2.kolih(); ii++)
   {
    ...
    || SRAV(in2.ravno(),"400000000000",0) == 0 )  /*освобдждено от НДС*/
     {
...
rnnrd5_6_endras(dod5,proc_dot,0,ff_svod_naim,ff_xml);

if(ff_svod_dot != NULL)
  rnnrd5_6_endras(dod5,proc_dot,1,ff_svod_dot,ff_xml_dot);
