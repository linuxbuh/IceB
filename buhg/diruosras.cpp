/*$Id: diruosras.c,v 5.5 2014/01/31 12:11:33 sasa Exp $*/
/*01.02.2017	30.12.2011	Белых А.И.	diruosras.c
Список операций расхода в учёте основных средств
*/
#include <errno.h>
#include "buhg.h"

void diruosras_sap(int metka_rr,const char *poisk);
int diruosras_vz(const char *kod);
void diruosras_kk(const char *skod,const char *nkod);
int diruosras_pvu(const char *kod);
void diruosras_ras(class SQLCURSOR *cur,const char *poisk);


int diruosras(int metka_rr, /*0-ввод и корректировка 1-выбор 2-чистый выбор*/
...
if(metka_rr == 2)
 {
  Y=X=-1;
  koment.new_plus(gettext("Выбприте операцию и нажмите \"Enter\""));
 }
...
lll:;
  ...
  diruosras_sap(metka_rr,poisk.ravno());
...
switch (K)
 {
  ...
  case ENTER: 
    if(metka_rr != 0)
     {
      sprintf(strsql,"select naik from Uosras where kod='%s'",kod_opv.ravno());
  ...
  case SFK2: 
    ...
    diruosras_vz(kod_opv.ravno());
    goto naz1;

  case FK3:       /*Удалить запись*/
    if(diruosras_pvu(kod_opv.ravno()) != 0)
     goto naz;
    ...
    sprintf(strsql,"delete from Uosras where kod='%s'",kod_opv.ravno());
...
int diruosras_vz(const char *kod_opv)
{
char strsql[512];
SQL_str row;
class SQLCURSOR cur;
...
if(kod_opv[0] != '\0') /*Корректировка записи*/
 {
  sprintf(strsql,"select * from Uosras where kod='%s'",kod_opv);
  if(readkey(strsql,&row,&cur) != 1)
   {
  ...
  if(metka_vido == 2)
   vido.new_plus(gettext("Изменения стоимости"));

  metka_prov=atoi(row[5]);
  if(metka_prov == 0)
    yesno.new_plus(gettext("Да"));
  else
    yesno.new_plus(gettext("Нет"));
...
VV.VVOD_SPISOK_add_MD(gettext("Код контра-та."));//4

VV.VVOD_SPISOK_add_data(kodn.ravno(),16);//0
VV.VVOD_SPISOK_add_data(naim.ravno(),60);//1
VV.VVOD_SPISOK_add_data(vido.ravno(),60);//2
VV.VVOD_SPISOK_add_data(yesno.ravno(),60);//3
VV.VVOD_SPISOK_add_data(kod_kontr.ravno(),16);//4
    ...
    if(N == 2) /*Вид операции*/
     {
      ...
      MENUW.VVOD_SPISOK_add_MD(gettext("Изменения стоимости"));
      ...
      switch (metka_vido)
       {
        ...
        case 2 :
         VV.VVOD_SPISOK_zapis_data(2,gettext("Изменения стоимости"));
         break;
       }
...
sprintf(strsql,"select kodkon from Kontragent where kodkon='%s'",VV.data_ravno(4));
if(readkey(strsql) != 1)
 {
  sprintf(strsql,"%s %s %s!",gettext("Нет кода контрагента"),VV.data_ravno(4),gettext("в списке контрагентов"));
  iceb_t_soob(strsql);
  goto naz;
 }
...
if(SRAV(kod_opv,VV.data_ravno(0),0) != 0)
 {
  sprintf(strsql,"select kod from Uosras where kod='%s'",VV.data_ravno(0));
  if(readkey(strsql) == 1)
   {
...
if(kod_opv[0] == '\0') /*Ввод новой записи*/
 {
  sprintf(strsql,"insert into Uosras values ('%s','%s',%d,%ld,%d,%d,'%s')",
  VV.data_ravno(0),
...
else /*Корректировка существующей*/
 {
  ...
  sprintf(strsql,"update Uosras set \
kod='%s',\
naik='%s',\
ktoz=%d, \
vrem=%ld, \
...
if(metka_kor_kod == 1)
 {
  diruosras_kk(kod_opv,VV.data_ravno(0));
  if(metka_vido > 0)
   iceb_t_soob(gettext("Необходимо откорректировать соотвецтвующий код операции в приходных проводках!"));
 }
...
sprintf(strsql,"select oper from Prov where oper='%s' and tz=2 and kto='%s' limit 1",kod,ICEB_MP_UOS);
...
----------------------------------------------------------------------\n");
fprintf(ff,gettext("\
 Код |Наименование операции         |Вид операц| Проводки |Контрагент|\n"));
fprintf(ff,"\
----------------------------------------------------------------------\n");
