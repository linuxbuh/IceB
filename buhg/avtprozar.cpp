/*$Id: avtprozar.c,v 5.11 2014/03/31 06:06:15 sasa Exp $*/
/*08.06.2016	10.03.2011	Белых А.И.	avtprozar.c
Подпрограмма автоматического выполнения проводок для произвольной системы настроек
*/
#include "math.h"
#include "buhg.h"

void avtprozar_vpnu(int tbn,short mp,short gp,short denz,double suma,const char *koment,const char *kod_kontr_baz,int kekv,const char *shet_v_kar,const char *shet_nu,const char *str_nast,class SPISOK *sp_prov,class masiv_din_double *sum_prov_dk,FILE *ff_prot);
void avtprozar_soc(int tbn,short mp,short gp,int kekv,const char *kod_kontr_baz,const char *shet_v_kar,class SPISOK *sp_prov,class masiv_din_double *sum_prov_dk,FILE *ff_prot);
...
class iceb_tu_str pnom1("");
class iceb_tu_str shet_zar_poum("");
class iceb_tu_str shet_zar_poum_bud("");
class iceb_tu_str shet_nu("");
class iceb_tu_str knah("");
class iceb_tu_str tabnom("");
class iceb_tu_str koment_v_alx("");
class SPISOK sp_prov;
class masiv_din_double sum_prov_dk;
class iceb_tu_str kontr_shet_zar_poum("");
class iceb_tu_str kontr_shet_kor("");
class iceb_tu_str kod_kontr_baz("");
class iceb_tu_str shet_v_kar("");

int kekv=0;
int kolpr=2;
char strsql[1024];
short dd=0,md=0,gd=0; /*Дата документа - все проводки имеют дату документа последним днём месяца*/
short dpr=0,mpr=0,gpr=0; /*Дата проводки - дата начисления или удержания*/
...
if(udprgr(ICEB_MP_ZARP,dd,md,gd,nomdok,0,0) != 0)
 return;

/*Если не введены счета для выполнения проводок по умолчанию, то проводки делать не нужно*/
if(iceb_t_poldan("Счёт для выполнения проводок по умолчанию",&shet_zar_poum,"avtprozar1.alx") != 0)
 {
  if(ff_prot != NULL)
...
if(iceb_t_poldan("Счёт для выполнения проводок по умолчанию",&shet_zar_poum,"avtprozar2.alx") != 0)
 {
  if(ff_prot != NULL)
...
if(kolstr <= 0)
 {
  if(ff_prot != NULL)
   fprintf(ff_prot,"%s-Не найдено ни одного начисления или удержания с ненулевой суммой!\n",__FUNCTION__);
  return;
 }
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  if(row[2][0] == '\0')
   {
    if(prn == 1)
     sprintf(strsql,"%s-%s %d %s %s!\n",__FUNCTION__,gettext("Табельный номер"),tbn,gettext("Не введён счёт в начислении с кодом"),row[4]);
    if(prn == 2)
     sprintf(strsql,"%s-%s %d %s %s!\n",__FUNCTION__,gettext("Табельный номер"),tbn,gettext("Не введён счёт в удержании с кодом"),row[4]);
    iceb_t_soob(strsql);
    continue;
   }
  ...
  rsdat(&dpr,&mpr,&gpr,row[0],2); /*дата выполнения проводки*/
  ...
  while(cur_alx.read_cursor(&row_alx) != 0)
   {
    ...
    if(polen(row_alx[0],&pnom1,1,'|') != 0)
     continue;

    if(SRAV("Счёт для выполнения проводок по умолчанию",pnom1.ravno(),0) == 0)
     {
      polen(row_alx[0],&shet_zar_poum,2,'|');
      if(ff_prot != NULL)
      ...
      if(provsh(&kontr_shet_zar_poum,shet_zar_poum.ravno(),&shetv,avkvs,0) != 0)
        return;
      continue;
     }    

    if(SRAV("Счёт для выполнения бюджетных проводок по умолчанию",pnom1.ravno(),0) == 0)
     {
      polen(row_alx[0],&shet_zar_poum_bud,2,'|');
      if(ff_prot != NULL)
       fprintf(ff_prot,"%s-Счёт для выполнения бюджетных проводок по умолчанию=%s\n",__FUNCTION__,shet_zar_poum.ravno());

      if(shet_zar_poum_bud.getdlinna() > 1)
       {
        class iceb_tu_str kontr_for_proverki(kod_kontr_baz.ravno());
        if(provsh(&kontr_for_proverki,shet_zar_poum.ravno(),&shetv,avkvs,0) != 0)
          return;
       }
      continue;
     }    
    ...
    if(metka_vp == 0)
     {
      if(SRAV("START",pnom1.ravno(),0) != 0)
       continue;

      if(ff_prot != NULL)
       fprintf(ff_prot,"%s",row_alx[0]);

      if(polen(row_alx[0],&knah,2,'|') != 0)
        continue;

      if(proverka(knah.ravno(),row[4],0,0) != 0)
       {
        if(ff_prot != NULL)
      ...
      if(polen(row_alx[0],&shet_nu,3,'|') != 0)
        continue;

      if(proverka(shet_nu.ravno(),row[2],1,0) != 0) /*если счёт не введён то подходит для всех счетов этого начиления/удержания*/
       {
        if(ff_prot != NULL)
         fprintf(ff_prot,"%s-Не подходит счёт\n",__FUNCTION__);
        continue;
       }
      if(polen(row_alx[0],&tabnom,4,'|') == 0)
       if(proverka(tabnom.ravno(),tbn,0,0) != 0)
        {
         if(ff_prot != NULL)
          fprintf(ff_prot,"%s-Не подходит табельный номер\n",__FUNCTION__);
         continue;
        }
      if(polen(row_alx[0],&koment_v_alx,5,'|') == 0)
       if(proverka(koment_v_alx.ravno(),row[5],4,0) != 0)
        {
         if(ff_prot != NULL)
    ...
    if(SRAV("START",row_alx[0],1) == 0)
       break;
    avtprozar_vpnu(tbn,mpr,gpr,dpr,suma,row[5],kod_kontr_baz.ravno(),kekv,shet_v_kar.ravno(),row[2],row_alx[0],&sp_prov,&sum_prov_dk,ff_prot);
  ...
  if(metka_vp == 0) /*выполняем проводку по умолчанию*/
   {
    ...
    class iceb_tu_str shet_for_prov(shet_zar_poum.ravno());
    ...
    if(prn == 1)
     ...
     {
      if(proverka(shetbu,row[2],0,1) == 0)
        if(shet_zar_poum_bud.getdlinna() > 1)
         shet_for_prov.new_plus(shet_zar_poum_bud.ravno());
     }

    kontr_shet_kor.new_plus(kod_kontr_baz.ravno());
    if(provsh(&kontr_shet_kor,row[2],&shetv,avkvs,0) != 0)
      return;

    kontr_shet_zar_poum.new_plus(kod_kontr_baz.ravno());
    if(provsh(&kontr_shet_zar_poum,shet_for_prov.ravno(),&shetv,avkvs,0) != 0)
      return;
    ...
    if(prn == 1)
      zapmpr(gpr,mpr,dpr,row[2],shet_for_prov.ravno(),kontr_shet_kor.ravno(),kontr_shet_zar_poum.ravno(),suma,0.,row[5],kolpr,kekv,&sp_prov,&sum_prov_dk,ff_prot);
    else
      zapmpr(gpr,mpr,dpr,shet_for_prov.ravno(),row[2],kontr_shet_zar_poum.ravno(),kontr_shet_kor.ravno(),suma,0.,row[5],kolpr,kekv,&sp_prov,&sum_prov_dk,ff_prot);
   }   
 }
/*проводки на начисленные отчисления в соц.фонды*/
avtprozar_soc(tbn,mp,gp,kekv,kod_kontr_baz.ravno(),shet_v_kar.ravno(),&sp_prov,&sum_prov_dk,ff_prot);
...
zapmpr1(nomdok,"",0,time(NULL),ICEB_MP_ZARP,dd,md,gd,0,&sp_prov,&sum_prov_dk,ff_prot);
...
class iceb_tu_str kodkon_deb("");
class iceb_tu_str kodkon_kre("");
class iceb_tu_str pnom("");
class iceb_tu_str koment_prov(koment);
class iceb_tu_str kekv_prov("");
short avkvs=1;
if(ff_prot != NULL)
...
if(polen(str_nast,&shet_deb,1,'|') != 0)
 {
  if(ff_prot != NULL)
   fprintf(ff_prot,"%s-Не найден счёт дебета!\n",__FUNCTION__);
  return;
 }
if(polen(str_nast,&shet_kre,2,'|') != 0)
 {
  if(ff_prot != NULL)
...
if(SRAV(shet_deb.ravno(),"SHET_V_KAR",0) == 0)
 shet_deb.new_plus(shet_v_kar);

if(SRAV(shet_deb.ravno(),"SHET_V_NU",0) == 0)
 shet_deb.new_plus(shet_nu);

if(SRAV(shet_kre.ravno(),"SHET_V_KAR",0) == 0)
 shet_kre.new_plus(shet_v_kar);

if(SRAV(shet_kre.ravno(),"SHET_V_NU",0) == 0)
 shet_kre.new_plus(shet_nu);
 
pnom.new_plus(shet_deb.ravno());
if(polen(pnom.ravno(),&shet_deb,1,':') == 0)
  polen(pnom.ravno(),&kodkon_deb,2,':');
else
 shet_deb.new_plus(pnom.ravno());

pnom.new_plus(shet_kre.ravno());
if(polen(pnom.ravno(),&shet_kre,1,':') == 0)
  polen(pnom.ravno(),&kodkon_kre,2,':');
else
 shet_kre.new_plus(pnom.ravno());

polen(str_nast,&kekv_prov,3,'|');
if(kekv_prov.getdlinna() <= 1)
 kekv_prov.new_plus(kekv);

polen(str_nast,&koment_prov,4,'|');
if(koment[0] != '\0')
...
if(provsh(&kodkon_deb,shet_deb.ravno(),&shetv,avkvs,0) != 0)
  return;

if(kodkon_kre.getdlinna() <= 1)
  kodkon_kre.new_plus(kod_kontr_baz);

if(provsh(&kodkon_kre,shet_kre.ravno(),&shetv,avkvs,0) != 0)
  return;

int kolpr=2;

zapmpr(gp,mp,denz,shet_deb.ravno(),shet_kre.ravno(),kodkon_deb.ravno(),kodkon_kre.ravno(),suma,0.,koment_prov.ravno(),kolpr,kekv_prov.ravno_atoi(),sp_prov,sum_prov_dk,ff_prot);
...
class iceb_tu_str kontr_for_soc(kod_kontr_baz);
class iceb_tu_str kontr_for_nah(kod_kontr_baz);
class iceb_tu_str koment_v_alx("");
int kodz=0;
int metka_bol=0;
...
//    printw("%s = %.2f\n",NAHIS.ravno(nomz),sum);
    ...
    if(proverka(kodnv.ravno(),kodnah.ravno(),0,1) == 0)
     continue;

    rsdat(&d,&m,&g,dataz,2);


    class iceb_tu_str shet_v_nah("");
    polen(NAHIS.ravno(nomz),&shet_v_nah,2,'|');
    kontr_for_nah.new_plus(kod_kontr_baz);
 
    if(provsh(&kontr_for_nah,shet_v_nah.ravno(),&shetv,avkvs,0) != 0)
      continue;
    
    class iceb_tu_str koment_v_nah("");
    
    polen(NAHIS.ravno(nomz),&koment_v_nah,4,'|');
    double deb=0.;
...
%.2f*%.2f/%.2f=%.2f\n",
    ...
    int metka_vp=0;
    class iceb_tu_str kod_fonda("");
    class iceb_tu_str shet_v_nast("");
    class iceb_tu_str tabnom_v_nast("");
    metka_vp=0;
    ...
    while(cur_alx.read_cursor(&row_alx) != 0)
     {
      ...
      if(metka_vp == 0)
       {
        ...
        if(polen(row_alx[0],&kod_fonda,2,'|') != 0)
         continue;

        if(proverka(kod_fonda.ravno(),kodz,0,0) != 0)
         {
          if(ff_prot != NULL)
        ...
        if(polen(row_alx[0],&shet_v_nast,3,'|') != 0)
         continue;

        if(proverka(shet_v_nast.ravno(),shet_v_nah.ravno(),0,0) != 0) /*если счёт не введён то подходит для всех счетов этого начисления*/
         {
          if(ff_prot != NULL)
        ...
        if(polen(row_alx[0],&tabnom_v_nast,4,'|') == 0)
         if(proverka(tabnom_v_nast.ravno(),tbn,0,0) != 0)
          {
           if(ff_prot != NULL)
        ...
        if(polen(row_alx[0],&koment_v_alx,5,'|') == 0)
         if(proverka(koment_v_alx.ravno(),koment_v_nah.ravno(),4,0) != 0)
          {
           if(ff_prot != NULL)
            fprintf(ff_prot,"%s-Не подходит коментарий\n",__FUNCTION__);
           continue;
      ...
      if(SRAV("START",row_alx[0],1) == 0)
         break;
      avtprozar_vpnu(tbn,m,g,d,deb,koment_v_nah.ravno(),kod_kontr_baz,kekv,shet_v_kar,row[1],row_alx[0],sp_prov,sum_prov_dk,ff_prot);
     }
    ...
    if(metka_vp == 0) /*выполняем проводку по умолчанию*/
     {
      ...
      class iceb_tu_str shet_v_fonde("");
      class iceb_tu_str shet_v_fonde_bud("");
     
      if(polen(row[1],&shet_v_fonde,1,',') != 0)
       shet_v_fonde.new_plus(row[1]);
      else
       polen(row[1],&shet_v_fonde_bud,2,',');

      if(shet_v_fonde_bud.getdlinna() > 1)
        if(proverka(shetb,shet_v_nah.ravno(),0,1) == 0)
          shet_v_fonde.new_plus(shet_v_fonde_bud.ravno());            

      if(provsh(&kontr_for_soc,shet_v_fonde.ravno(),&shetv,avkvs,0) != 0)
        continue;

      zapmpr(g,m,d,shet_v_nah.ravno(),shet_v_fonde.ravno(),kontr_for_nah.ravno(),kontr_for_soc.ravno(),deb,0.,koment_v_nah.ravno(),kolpr,kekv,sp_prov,sum_prov_dk,ff_prot);
     }   
