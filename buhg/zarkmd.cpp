...
/*03.04.2020	05.05.2004	Белых А.И.	zarkmd.c
...
int podr,
const char *nah_only,
int metka_r, /*0-расчёт по отработанным дням 1-расчёт по отработанным часам*/
...
class iceb_tu_str kod_prem_z_god("");
iceb_t_poldan("Коды премии за год",&kod_prem_z_god,"zarnast.alx",0);
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  if(proverka(kod_prem_z_god.ravno(),row[0],0,1) == 0)
   {
    sprintf(strsql,"%s-%3s %10s %s\n",__FUNCTION__,row[0],row[1],row[2]);
    zar_pr_ins(strsql);
