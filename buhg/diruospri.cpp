/*$Id: diruospri.c,v 5.5 2014/01/31 12:11:33 sasa Exp $*/
/*01.02.2017	25.01.2017	Белых А.И.	diruospri.c
Список операций расхода в учёте основных средств
*/
#include <errno.h>
#include "buhg.h"

void diruospri_sap(int metka_rr,const char *poisk);
int diruospri_vz(const char *kod);
void diruospri_kk(const char *skod,const char *nkod);
int diruospri_pvu(const char *kod);
void diruospri_ras(class SQLCURSOR *cur,const char *poisk);


int diruospri(int metka_rr, /*0-ввод и корректировка 1-выбор 2-чистый выбор*/
...
lll:;
  ...
  diruospri_sap(metka_rr,poisk.ravno());
...
switch (K)
 {
  ...
  case ENTER: 
    if(metka_rr != 0)
     {
      sprintf(strsql,"select naik from Uospri where kod='%s'",kod_opv.ravno());
  ...
  case SFK2: 
    ...
    diruospri_vz(kod_opv.ravno());
    goto naz1;

  case FK3:       /*Удалить запись*/
    if(diruospri_pvu(kod_opv.ravno()) != 0)
     goto naz;
    ...
    sprintf(strsql,"delete from Uospri where kod='%s'",kod_opv.ravno());
...
int diruospri_vz(const char *kod_opv)
{
char strsql[512];
SQL_str row;
class SQLCURSOR cur;
...
if(kod_opv[0] != '\0') /*Корректировка записи*/
 {
  sprintf(strsql,"select * from Uospri where kod='%s'",kod_opv);
  if(readkey(strsql,&row,&cur) != 1)
   {
...
if(SRAV(kod_opv,VV.data_ravno(0),0) != 0)
 {
  sprintf(strsql,"select kod from Uospri where kod='%s'",VV.data_ravno(0));
  if(readkey(strsql) == 1)
   {
...
if(kod_opv[0] == '\0') /*Ввод новой записи*/
 {
  sprintf(strsql,"insert into Uospri values ('%s','%s',%d,%ld,%d,%d)",
  VV.data_ravno(0),
...
else /*Корректировка существующей*/
 {
  ...
  sprintf(strsql,"update Uospri set \
kod='%s',\
naik='%s',\
ktoz=%d, \
vrem=%ld, \
...
if(metka_kor_kod == 1)
 {
  diruospri_kk(kod_opv,VV.data_ravno(0));
  if(metka_vido > 0)
   iceb_t_soob(gettext("Необходимо откоректировать соотвецтвующий код операции в расходных операциях!"));
 }

if(kod_opv[0] == '\0')
 if(metka_vido > 0) /*Пишем парную запись в приходные операции*/
  {
   sprintf(strsql,"insert into Uosras (kod,naik,ktoz,vrem,vido,prov,kk) values('%s','%s',%d,%ld,%d,%d,'00')",
   VV.data_ravno(0),
   VV.data_ravno_filtr(1),
   iceb_t_getuid(),time(NULL),
   metka_vido,
...
sprintf(strsql,"select oper from Prov where oper='%s' and tz=1 and kto='%s' limit 1",kod,ICEB_MP_UOS);
...
-----------------------------------------------------------\n");
fprintf(ff,gettext("\
 Код |Наименование операции         |Вид операц| Проводки |\n"));
fprintf(ff,"\
-----------------------------------------------------------\n");
...
while(cur->read_cursor(&row) != 0)
 {
  ...
  if(atoi(row[4]) == 1)
    vido.new_plus(gettext("Внутренняя"));
  if(atoi(row[4]) == 2)
    vido.new_plus(gettext("Изм. стоимости"));
