/*$Id: iceb_tu_propis.c,v 5.5 2013/10/09 07:05:14 sasa Exp $*/
/*16.04.2017	05.07.2011	Белых А.И.	iceb_tu_propis.c
...
const char *iceb_tu_propis(double hs,int metka_1s) /*0-первая буква маленькая 1-большая*/
{
static class iceb_tu_str str("");
...
if(metka_1s == 0)
 return(iceb_tu_adrsimv(1,str.ravno()));/*первый символ всегда пробел*/
...
if(g_utf8_validate(str.ravno(),-1,NULL) == FALSE)
...
else
 {
  ...
  gchar *strout=g_strdup_printf("%s%s",s1.ravno(),iceb_tu_adrsimv(2,str.ravno()));
  str.new_plus(strout);
  g_free(strout);  
 }
