...
/*06.12.2018	17.03.2016	Белых А.И.	iceb_t_nn10_end_xml.cpp
Концовка файла с налоговой накладной в формате xml
*/
#include "buhl.h"

void iceb_t_nn10_end_xml(const char *kodop,
...
class SQLCURSOR cur;
char strsql[512];
class iceb_tu_str fio(iceb_t_getfioop());
  ...
  if(atoi(row[0]) != 0)
   {
    ...
    if(iceb_t_get_rtn(atoi(row[0]),&rp) == 0)
     {
