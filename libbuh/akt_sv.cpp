...
/*05.11.2009	26.01.2007	Белых А.И.	akt_sv.c
...
void akt_sv(class akt_sverki *akt_svr,FILE *ff, /*с месячными итогами*/
FILE *ff_bi) /*Без месячных итогов*/
{

zagsh(akt_svr->shet.ravno(),ff);
zagsh(akt_svr->shet.ravno(),ff_bi);
...
for(int ii=0; ii < akt_svr->data_nomd.kolih(); ii++)
 {
  rsdat(&d,&m,&g,akt_svr->data_nomd.ravno(ii),1);
  if(mz != m || gz != g)
   {
    if(mz != 0)
  ...
  d,m,g,
  iceb_tu_kolbait(20,akt_svr->koment.ravno(ii)),iceb_tu_kolbait(20,akt_svr->koment.ravno(ii)),akt_svr->koment.ravno(ii),
  deb,kre);
...
if(deb_ob_mes != 0. || kre_ob_mes != 0.)
 {
  ...
  iceb_tu_kolbait(31,gettext("Оборот за месяц")),gettext("Оборот за месяц"),deb_ob_mes,kre_ob_mes);
 }
