...
/*06.12.2018	17.03.2016	Белых А.И.	iceb_t_nn10_zag_xml.c
Заголовок налоговой накладной в формате xml.
*/
#include "buhl.h"

void iceb_t_nn10_zag_xml(int tipz,
const char *nomdok,
...
class iceb_tu_str inn_kontr00("");
class iceb_tu_str kod_filiala00("");
class iceb_tu_str kod_edrpu00("");
class iceb_tu_str naim_kontr("");
class iceb_tu_str inn_kontr("");
class iceb_tu_str kod_filiala("");
class iceb_tu_str kod_edrpu("");
...
if(readkey(strsql,&row,&cur) >= 1)
  polen(row[0],&kod_filiala,2,'|');

fprintf(ff_xml," <DECLARBODY>\n");

class iceb_tu_str tip_prih("");
if(polen(vid_dok,&tip_prih,2,',') == 0)
 {
...
if(tipz == 1)
 {
  ...
  fprintf(ff_xml,"  <HNAMEBUY>%s</HNAMEBUY>\n",iceb_tu_filtr_xml(naim_kontr.ravno()));

  if(SRAV(inn_kontr00.ravno(),"100000000000",0) == 0 || SRAV(vid_dok,"ПНЕ,02",0) == 0)
   fprintf(ff_xml,"  <HNAMESEL>Неплатник</HNAMESEL>\n");
  else
   fprintf(ff_xml,"  <HNAMESEL>%s</HNAMESEL>\n",iceb_tu_filtr_xml(naim_kontr00.ravno()));
  ...
  SRAV(inn_kontr.ravno(),"100000000000",0) != 0 && 
  SRAV(inn_kontr.ravno(),"300000000000",0) != 0  && 
  SRAV(inn_kontr.ravno(),"400000000000",0) != 0  &&
  SRAV(inn_kontr.ravno(),"600000000000",0) != 0 )
     fprintf(ff_xml,"  <HTINSEL>%s</HTINSEL>\n",kod_edrpu.ravno());
    
  if(kod_edrpu00.getdlinna() > 1 &&
  SRAV(inn_kontr00.ravno(),"100000000000",0) != 0 && 
  SRAV(inn_kontr00.ravno(),"300000000000",0) != 0  && 
  SRAV(inn_kontr00.ravno(),"400000000000",0) != 0  &&
  SRAV(inn_kontr00.ravno(),"600000000000",0) != 0 )
...
if(tipz == 2)
 {
  ...
  if(SRAV(inn_kontr.ravno(),"100000000000",0) == 0 || SRAV(vid_dok,"ПНЕ,02",0) == 0)
