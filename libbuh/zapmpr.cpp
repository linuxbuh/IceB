/* $Id: zapmpr.c,v 5.23 2013/09/26 09:43:56 sasa Exp $ */
/*01.03.2011	32.09.1998	Белых А.И.	zapmpr.c
...
if(fabs(deb) > 0.009 && fabs(kre) > 0.009)
 {
  VVOD SOOB(1);
  sprintf(bros,"%s-%sт (%.2f) %s (%.2f) %s !",__FUNCTION__,gettext("Дебет"),deb,gettext("и кредит"),kre,gettext("не равны нолю"));
  SOOB.VVOD_SPISOK_add_MD(bros);
  SOOB.VVOD_SPISOK_add_MD(gettext("Попытка сделать логически не правильную проводку!"));
