/*$Id:$*/
/*22.03.2017	22.03.2017	Белых А.И.	iceb_t_nn9_start_s.c
...
if(fil_ff.start(imaf,"r") != 0)
 return;

sprintf(imaf_tmp,"nn9%d.tmp",getpid());
class iceb_fopen fil_tmp;

if(fil_tmp.start(imaf_tmp,"w") != 0)
 return;
int nom_str=0;
int nom_sum=0;

while(iceb_tu_fgets(&stroka,fil_ff.ff) == 0)
 {
     ...
     if(suma[nom_sum] != 0.)
      {
       ...
       iceb_tu_vstav(&stroka,strsql,163,181,1);
      }
     nom_sum++;
     break;
   }
