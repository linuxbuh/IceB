/* $Id: prosprus.c,v 5.31 2011-01-13 13:43:22 sasa1 Exp $ */
/*01.11.2009    22.02.1994      Белых А.И.      prosprus.c
...
short provprus(char sh1[],short d,short m,short g,const char nn[],int tp,int podr,const char matshet[]);
...
if(mr == 2)
 {
  ...
  provprus(sh1,d,m,g,nn,tp,podr,matshet);
  ...
  for(i=0; i < spsh.kolih() ; i++)
   {
    ...
    move(yv++,xv);
    printw("%-*s %14s",iceb_tu_kolbait(8,sh1),sh1,prnbr(sumpokart.ravno(i)));
...
and podr=%d and nomd='%s' and tp=%d",g,m,d,podr,nn,tp);
  ...
  */
  ...
  mop=mpi=0;
  memset(matshet,'\0',sizeof(matshet));
  mop=provprus(sh1,d,m,g,nn,tp,podr,matshet);
...
//    sv=0;
    if(matshet[0] != '\0')
     {
      sprintf(strsql,"%s %s!",
      gettext("В документе нет услуги со счетом"),
      matshet);
        ...
        if(fabs(sp.ravno(i) - sumpokart.ravno(i)) > 0.009)
         {
          ...
          sh1,sp.ravno(i),sumpokart.ravno(i),i,spsh.kolih());
          SOOB.VVOD_SPISOK_add_MD(strsql);
          mpi++;
         }
       }
        ...
        if(fabs(sp.ravno(i) - sumasnds) > 0.009)
         {
          ...
          sh1,sp.ravno(i),sumasnds,i,spsh.kolih(),sumkor);
          SOOB.VVOD_SPISOK_add_MD(strsql);
...
//sprintf(bros,"%s",iceb_t_imafnsi("slugbl.alx"));
if(iceb_t_pbpds(m,g) == 0) //Дата не заблокирована
