/*$Id: zaphkor.c,v 5.17 2013/05/17 14:56:16 sasa Exp $*/
/*29.06.2009	15.01.2003 	Белых А.И.	zaphkorf.c
...
int   zaphkor(int mkor, //0-новая запись 1-корректировка
const char *kassa,
int tipz,
...
if(sql_zap(&bd,strsql) != 0)
 {
  if(sql_nerror(&bd) == ER_DUP_ENTRY) //Запись уже есть
   ...
   {
    msql_error(&bd,"zaphkor-Ошибка записи !",strsql);
    return(2);
   } 
 }
...
and tp=%d",
  g,m,d,g,m,d,g,iceb_t_getuid(),vrem,
...
ktoi=%d,\
vrem=%ld \
where datd='%04d-%02d-%02d' and nomd='%s' and kassa=%s \
and tp=%d",
  g,m,d,g,iceb_t_getuid(),vrem,
...
if(SRAV(nomdz,nomd,0) != 0 || SRAV(kassa,kassaz,0) != 0)
 {
  sprintf(strsql,"update Kasord1 set \
nomd='%s',\
kassa=%s,\
...
and tp=%d",nomd,kassa,iceb_t_getuid(),vrem,
  g,nomdz,kassaz,tipz);
...
ktoi=%d,\
vrem=%ld \
where datd='%04d-%02d-%02d' and datp='%04d-%02d-%02d' and nomd='%s' and \
pod=%d and kto='%s'",
  kodop,nomd,g,m,d,g,m,d,atoi(kassa),iceb_t_getuid(),vrem,
...
pod=%d and kto='%s'",
kodop,nomd,g,m,d,atoi(kassa),iceb_t_getuid(),vrem,
