/*$Id: imp_kasord.c,v 5.30 2013/05/17 14:56:02 sasa Exp $*/
/*22.02.2016	28.03.2007	Белых А.И.	imp_kasord_rf.c
...
void avtprkas(const char *kassa,short tipz,const char *nomd,short dd,short md,short gd,const char *shetk,const char *shetd,const char *kodop);
...
int imp_kasord_r_vp(short dd,short md,short gd,const char *kassa,int tipzap,const char *nomdok,const char *kodop,const char *shetd,int *kol_oh);

int imp_kasord_rf(const char *imafim,int metka) //0-проверка на ошибки 1- загрузка
{
if(metka == 0)
...
if(stat(imafim,&work) != 0)
 {
  printf("%s %s!\n",gettext("Нет такого файла"),imafim);
  return(1);
 }
...
while(iceb_tu_fgets(&stroka,ffimp.ff) == 0)
 {
  ...
  if(SRAV("KDOK",kod_str.ravno(),0) == 0) //Строка с шапкой документа
   {
    if(nomdok.getdlinna() > 1)/*подтверждаем документ и выполняем проводки*/
     imp_kasord_r_vp(dd,md,gd,kassa.ravno(),tipzap,nomdok.ravno(),kodop.ravno(),shetd.ravno(),&kol_oh);
    ...
    if(rsdat(&dd,&md,&gd,bros.ravno(),1) != 0)
     {
      ...
      printf("%s\n",gettext("Не правильно введена дата !"));
      kol_oh++;
     }
    ...
    if(metka == 0) //проверка на ошибки
     {
      ...
      if(readkey(strsql,&row,&cur) != 1)
       {
        printf("%s %s !\n",gettext("Не найден код кассы"),kassa.ravno());
        kol_oh++;
       }
      ...
      if(nomdok.ravno()[0] != '\0')
       {
        ...
        if(readkey(strsql,&row,&cur) >= 1)
         {
          printf("%s %s %s !\n",gettext("Номер документа"),nomdok.ravno(),gettext("уже есть в базе"));
          kol_oh++;
         }
      ...
      if(nomer_bl.getdlinna() > 1)
       {
        ...
        if(readkey(strsql,&row,&cur) >= 1)
         {
          printf("%s %s %s %s!\n",gettext("Номер бланка"),nomer_bl.ravno(),gettext("уже имеет документ с номером"),row[0]);
          kol_oh++;
         }
    ...
    if(metka == 1) //запись в базу шапки документа
     {
      ...
      if(nomdok.ravno()[0] == '\0')
          nomnak(gd,kassa.ravno(),&nomdok,tipzap,2,1);
  ...
  if(SRAV("KZAP",kod_str.ravno(),0) == 0 ) //Строка содержимым документа
   {
      ...
      if(readkey(strsql,&row,&cur) == 1)
       {
        if(zapzkor(1,kassa.ravno(),tipzap,dd,md,gd,nomdok.ravno(),kontr.ravno_filtr(),suma+atof(row[0]),kontr.ravno_filtr(),"") != 0)
         {
          printf("%s\n",gettext("Ошибка корректировки записи в кассовом ордере"));
          kol_oh++;
          continue;
         }
       }
       ...
       if(zapzkor(0,kassa.ravno(),tipzap,dd,md,gd,nomdok.ravno(),kontr.ravno_filtr(),suma,"","") != 0)
        {
         printf("%s\n",gettext("Ошибка записи в кассовый ордер"));
         kol_oh++;
         continue;
        }
...
if(metka == 1)
 {
  ...
  printf("%s %d %s %.2f\n",gettext("Приходных документов"),koldok_p,gettext("на сумму"),i_sumap);

  printf("%s %d %s %.2f\n",gettext("Расходных документов"),koldok_r,gettext("на сумму"),i_sumar);
...
if(shetk.ravno()[0] == '\0')
 {
  ...
  if(readkey(strsql,&row,&cur) != 1)
   {
    printf("%s %s !\n",gettext("Не найдена касса"),kassa);
    return(1);
   }
  else
   shetk.new_plus(row[0]);
